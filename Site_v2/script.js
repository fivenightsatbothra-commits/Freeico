    ...
    if (mainSearch) {
        mainSearch.addEventListener('focus', () => {
            if (!searchIndexData) {
                ...
                tempScript.src = 'https://cdn.jsdelivr.net/gh/fivenightsatbothra-commits/Freeico@main/Site/data/search-index.js';
...
function renderCollectionView(collection) {
    ...
    if (window.collectionData && window.collectionData[collection.id]) {
        ...
        if (!document.getElementById(scriptId)) {
            ...
            script.id = scriptId;
            script.src = `https://cdn.jsdelivr.net/gh/fivenightsatbothra-commits/Freeico@main/Site/data/${collection.id}.js`;
            script.onerror = () => {
...
function renderIconsGrid(collection, files) {
    ...
    const renderChunk = () => {
        ...
        const html = chunk.map((file, idx) => `
            <div class="icon-item" data-id="search-icon-${file.name}" data-filename="${file.name}" data-collection="${file.customCollection || collection.name}" data-author="${file.customAuthor || collection.author}">
                ${file.content}
            ...
            try {
                ...
                if (!document.getElementById(scriptId)) {
                    await new Promise((resolve, reject) => {
                        const script = document.createElement('script');
                        script.id = scriptId;
                        script.src = `https://cdn.jsdelivr.net/gh/fivenightsatbothra-commits/Freeico@main/Site/data/${colId}.js`;
